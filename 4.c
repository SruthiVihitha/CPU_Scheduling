...
GtkWidget *pid_entry, *at_entry, *bt_entry, *output_label, *gantt_canvas, *treeview;
...
void simulate(GtkWidget *widget, gpointer data) {
    ...
    while (completed < process_count) {
        int idx = -1;
        int min_bt = INT_MAX; // Use INT_MAX from limits.h

        for (int i = 0; i < process_count; i++) {
            if (processes[i].at <= st && !processes[i].is_completed && processes[i].bt < min_bt) {
...
gboolean on_draw_event(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
    int scale_factor = 40; // Increase horizontal scale for each time unit
    int rect_height = 50;  // Increase rectangle height
...
int main(int argc, char *argv[]) {
    ...
    list_store = gtk_list_store_new(6, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT, G_TYPE_INT);
    treeview = gtk_tree_view_new_with_model(GTK_TREE_MODEL(list_store));

    GtkCellRenderer *renderer = gtk_cell_renderer_text_new();
    for (int i = 0; i < 6; i++) {
        GtkTreeViewColumn *column = gtk_tree_view_column_new_with_attributes(
            (i == 0) ? "PID" : (i == 1) ? "AT" : (i == 2) ? "BT" : (i == 3) ? "CT" : (i == 4) ? "TAT" : "WT",
    ...
    gtk_grid_attach(GTK_GRID(grid), simulate_button, 0, 4, 2, 1);
    gtk_grid_attach(GTK_GRID(grid), output_label, 0, 5, 2, 1);
    gtk_grid_attach(GTK_GRID(grid), gantt_canvas, 0, 6, 2, 1);
    gtk_grid_attach(GTK_GRID(grid), treeview, 0, 7, 2, 1);
